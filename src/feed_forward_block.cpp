// ============================================================
// src/feed_forward_block.cpp  (FILE BARU)
// ============================================================
#include "feed_forward_block.h"
#include <cmath>

// GELU (tanh approximation) — pilihan standar di GPT-2, lebih halus dari ReLU
Scalar FeedForwardBlock::gelu_scalar(Scalar x) {
    const Scalar c = 0.7978845608f; // sqrt(2/pi)
    Scalar x3 = x * x * x;
...
{
    Scalar std1 = std::sqrt(2.0f / static_cast<Scalar>(embed_dim));  // He (menuju GELU ~ ReLU-like)
...
Tensor3D FeedForwardBlock::forward(const Tensor3D& input) {
    ...
    Tensor3D z1 = input.batched_matmul(W1_).add_bias(b1_); // batch x seq x hidden_dim
    ...
    return a1.batched_matmul(W2_).add_bias(b2_); // batch x seq x embed_dim
}
...
Tensor3D FeedForwardBlock::backward(const Tensor3D& grad_output) {
    ...
    for (size_t b = 0; b < batch; ++b) {
        Matrix a1_b = post_activation_cache_.slice_batch(b);   // seq x hidden_dim
        Matrix go_b = grad_output.slice_batch(b);               // seq x embed_dim
    ...
    // --- Grad terhadap a1 (post-activation), lalu ke z1 (pre-activation) lewat turunan GELU ---
    Matrix W2_t = W2_.transpose();
    Tensor3D grad_a1 = grad_output.batched_matmul(W2_t); // batch x seq x hidden_dim
    ...
    for (size_t b = 0; b < batch; ++b) {
        Matrix input_b = input_cache_.slice_batch(b);   // seq x embed_dim
        Matrix gz1_b = grad_z1.slice_batch(b);            // seq x hidden_dim
    ...
    return grad_z1.batched_matmul(W1_t); // batch x seq x embed_dim
}
